from blog.models import User, login_manager, Post, LikePost, relationship, Comment, rstatus,messages, reply_message
from flask import render_template, url_for, redirect, request, flash, session
from blog.auth.authForms import PostForm, UpdateAccountInfo, CommentForm, CommentFormUpdate, UpdatePostForm,MessageForm,ReplyMessageForm
...
def new_post():
    ...
    if form.validate_on_submit():
        post = Post.insert(content=form.post_content.data,
...
def update_comment_post(pid, cid):
    ...
    if form.validate_on_submit():
        Comment.update_comment(
            form.comment_content_update.data, current_user.id, pid)
        flash('comment updated', 'success')
        return redirect(url_for('.view_post', pid=pid, cid = cid))
    if request.method == meth[1]:
...
def save_photo(form_photo):
    ...
    photo_path = os.path.join(
        current_app.root_path + '/static/photos/' + current_user.username, photo_name)
...
def account(username):
    ...
    if form.validate_on_submit():
        user = User.update(name=form.name.data, email=form.email.data,
        ...
        os.rename(os.path.join(current_app.root_path + '/static/photos/', current_user.username),
                  os.path.join(current_app.root_path + '/static/photos', form.username.data))
        user.execute()
...
@site.route('/friends', methods=meth)
@login_required
def friends():
	form = MessageForm()
	return object_list('home/friends.html', User.select().where(User.id != current_user.id),form = form, rela=relationship())

@site.route('/messages', methods=meth)
@login_required
def user_messages():
	return object_list('home/messages.html', messages.messages(),context_variable='message_list')

@site.route('/message/<int:mid>', methods=meth)
@login_required
def message(mid):
	msgs = messages.get_by_id(mid)
	form = ReplyMessageForm()
	messages.read_message(msgs.from_user.id)
	if form.validate_on_submit():
		reply_message.reply(form.content.data, current_user.id, mid)
		flash('reply sent','success')
		return redirect(url_for('.message', mid = mid))
	return render_template('home/message.html',msg = reply_message.to_message(), form= form)

@site.route('/send_message/<int:usid>', methods=meth)
@login_required
def send_message(usid):
	form = MessageForm()
	if form.validate_on_submit():
		messages.send(usid,form.content.data)
...
def confirm_friend(usid):
	relationship.accept_relationship(usid)
